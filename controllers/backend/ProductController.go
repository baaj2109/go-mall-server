...
func (c *ProductController) GoEdit() {
	...
	if err3 != nil {
		c.Error("修改數據失敗", "/product/edit?id="+strconv.Itoa(id))
		return
	}
	//5.修改圖庫數據 （增加）
	wg.Add(1)
	...
	//6.修改商品類型属性數據         1. 刪除當前商品id對應的類型属性  2. 執行增加
...
func (c *ProductController) Delete() {
	...
	c.Success("刪除商品成功", c.Ctx.Request.Referer())
}
