...
func (c *AuthController) GoAdd() {
	...
	if err != nil {
		c.Error("增加數據失敗", "/auth/add")
		return
	}
	c.Success("增加數據成功", "/auth")
}
...
func (c *AuthController) GoEdit() {
	...
	if err6 != nil {
		c.Error("修改權限失敗", "/auth/edit?id="+strconv.Itoa(id))
		return
	}
	c.Success("修改權限成功", "/auth")
}
...
func (c *AuthController) Delete() {
	...
	if auth.ModuleId == 0 {
		...
		if len(auth2) > 0 {
			c.Error("請刪除當前頂級模块下面的菜單或操作！", "/auth")
			return
		}
	}
