...
func (c *ProductTypeController) GoAdd() {
	...
	if err1 != nil {
		c.Error("增加商品類型失敗", "/productType/add")
	} else {
		c.Success("增加商品類型成功", "/productType")
	}
}
...
func (c *ProductTypeController) GoEdit() {
	...
	if title == "" {
		c.Error("標題不能為空", "/productType/edit?id="+strconv.Itoa(id))
		return
	}
	productType := models.ProductType{Id: id}
	models.DB.Find(&productType)
	...
	if err2 != nil {
		c.Error("修改商品類型失敗", "/productType/edit?id="+strconv.Itoa(id))
	} else {
		c.Success("修改商品類型成功", "/productType")
	}
}
